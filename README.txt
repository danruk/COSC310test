1. Make sure your terminal is in the backend directory 
2.  Create your virtual environment (name it whatever you want) IN THE BACKEND DIRECTORY
3. Activate the virtual environment
4. pip install "fastapi[standard]"
5. To run: fastapi dev app/main.py
6. You can view the docs link that pops up in your terminal to view teh endpoints and can play around with them there
7. To close the server/session spam ctl + c in terminal 
